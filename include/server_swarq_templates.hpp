 ...
 */
template <typename OIter>
size_t ServerSWARQ<OIter>::receive_frame(Frame &frame){
  ...
  size_t length;
  length = socket.receive_from(rcv_buffer, enpoint, 0 , error);
 ...
 */
template<typename OIter>
OIter ServerSWARQ<OIter>::receive_nbyte(OIter out, size_t nbyte){
  size_t nbyte_recv = 0;
